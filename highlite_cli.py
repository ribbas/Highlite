...
from highlite import _version, buzzwords, customcorpus, metrics, recreate, \
    stats, textio, RAWCORPUS_DIR, HTML_CONVERTED_PATH
...
if __name__ == "__main__":
    ...
    parser.add_argument("target_doc", metavar="<PATH TO INPUT FILE>", nargs="?",
                        help="| Input target document for analysis")
    ...
    parser.add_argument("--ignore_terms", nargs="+", metavar="TERM", default=[],
    ...
    parser.add_argument(
        ...
        help="| Generate an HTML version of the input document with the scored\
         terms tagged with heatmap palette"
    )
    ...
    results_path = ""
    if args.target_doc:
        results_path = path.basename(args.target_doc).rsplit(".")[0] \
            + "_results.json"
    ...
    if args.preview:
        ...
        import webbrowser
        webbrowser.get().open(HTML_CONVERTED_PATH)
